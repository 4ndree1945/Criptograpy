...
def des_encrypt_block(block64, subkeys):
    ...
    for i in range(16):
        newR = xor(L, feistel(R, subkeys[i]))
        L = R
        R = newR
...
if __name__ == "__main__":
    ...
    key_str = input("Masukkan key (maksimal 8 karakter): ")

    if len(key_str) > 8:
        print("Kunci harus maksimal 8 karakter!")
        exit()

    # padding key ke 8 char
    key_str = key_str.ljust(8, '\x00')

    # convert ke biner
    key_bin = str_to_bin(key_str)
    ...
    # proses plaintext block per block
    ...
    ciphertext_hex = f"{int(ciphertext_bin,2):X}"
